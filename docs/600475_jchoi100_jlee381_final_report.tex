...
On the USPS zipcode dataset, we also ran SVM-KNN, using algorithm 2 that involves a more 'accurate' distance, the tangent distance. The experiments were performed with values of $K = 1, 3, 5, 10$. Experiment with $K = 1$, which does not involve SVM, was performed as well since the nearest neighbor from $L_2$ distance may differ from that of tangent distance. The results are as follows:
\\
Highest accuracy was achieved at $K=3$ with the error rate of 3.79\%, performing better than the results from any of the experiments using $L_2$ distance, although lower than those of [1]. One possibility for the cause may be the 'smoothening' of the images mentioned in [1].
...
Some of the more 'accurate' distance functions were beyond the scope of our project. Shape contest required sampling points from the Canny edges, bipartite graph matching $(O(N^3))$, scaling of the image (ex: 28x28 to 70x70 for MNIST) and computing multiple distances, such as Appearance cost, Shape Context cost, and Transformation cost. Computing tangent distance on large data also caused memory issues, thus experiments with tangent distance on MNIST was not performed.
